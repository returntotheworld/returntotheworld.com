...
* @return:
*/
class RememberController extends AuthController{
			
	public function index(){
		$this->assign("remember","active open");
		$this->assign("rememberadd","class='active'");
		$this->tag = M('tag')->where(array("t_view"=>1))->select();
		$this->display();
	}
	
	public function rememberAdd(){
		if(!IS_AJAX){
	...
	public function rememberList(){
		$this->assign("remember","active open");
		$this->assign("rememberlist","class='active'");
		$Article	= M('remember'); 
		$count	= $Article->count();
		...
		public function rememberEdit(){
		$this->tag = M('tag')->where(array("t_view"=>1))->select();
		$this->assign("remember","active open");
		$this->assign("rememberlist","class='active'");
		$info = M('remember')->where(array("a_id"=>I('get.id')))->find();
		if(!$info)
			$this->error('参数错误!',0,0);
		else{
	...
	public function rememberDel(){
		if(!IS_AJAX){
			$this->error('提交方式不正确',0,0);
		}else{
			if(M('remember')->where(array("a_id"=>I('post.id')))->delete())
