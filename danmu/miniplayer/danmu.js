...
function initPlayer(_in_videoid) {
	...
	controlfuns.volumechange = function() {
		...
		if (core.player.video.muted) player.volumestat.innerHTML = 'O';
		else {
			player.volumestat.innerHTML = '<img src="jj.svg" alt="">';
		}
	}
	...
	function initevents() {
		...
		function(e) {
			...
			if (player.o.loop) {
				...
				player.loop.style.color = '#fd4c5d';
				tip("洗脑循环");
			} else {
				//console.log("取消循环");
				core.player.video.loop = false;
				player.loop.style.color = '#fff';
				tip("关闭循环");
			}
		});
