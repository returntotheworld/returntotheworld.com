...
if (isID($option[0])) {
	...
	Global $SQL;
	Global $flags;
	$stmt = $SQL->prepare('SELECT title,address,count,coveraddress,description,options FROM lt_video WHERE id=?');
	...
	$stmt->close();
	$SQL->query('UPDATE lt_video SET count=count+1 WHERE id='.$option[0]);
	//$SQL->close();
} else {
	echo 'Error';
